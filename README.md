...
├── 99_存档_只读/          ⛔ 历史存档，勿改勿引用（仅本地，未入库）| ⛔ historical archive, do not modify or reference (local-only, not published)
...
│   └── 迁移说明_20260825.txt
├── 系统控制台_latest        Linux 控制台根目录副本（快速访问；仅本地，未入库，见 .gitignore）| Linux console root copy (quick access; local-only, not published, see .gitignore)
```
...
> **🟩 2026-09-10 更正横幅 / Correction banner 2026-09-10：** 下表为 **v1 时代**的历史记录（两目录均已随 v2.0 同步移除，仅存历史提交）。当前唯一主版本为 `05_控制台_Linux/系统控制台_最新_20260909/`（v2.0）。本坑的教训（勿按目录名判断新旧）仍然有效。 / *The table below is v1-era history (both directories were removed in the v2.0 sync and survive only in old commits). The sole current master is `05_控制台_Linux/系统控制台_最新_20260909/` (v2.0). The lesson itself — never judge freshness by directory name — still stands.*
...
**日志 (Log)：** `~/.local/share/系统控制台/data/console.log`（v2.0 运行位置内相对路径 `data/console.log`；v1 曾为 `~/桌面/系统控制台/data/console.log`，桌面副本已于 2026-08-31 移除）| *(v2.0 runtime-relative `data/console.log`; v1 once used `~/桌面/系统控制台/data/console.log`, the desktop copy was removed on 2026-08-31)*
...
已安装至三处（桌面 + `~/.local/share/applications/` 供菜单搜索 + 程序目录自引用），均由 v2.0 `install.sh` 以 `APP_DIR` 动态生成（重跑安装即可修复路径）| Installed at three places (desktop + `~/.local/share/applications/` for menu search + an in-place self-reference copy), all dynamically generated by the v2.0 `install.sh` via `APP_DIR` (re-running the installer fixes paths)：
- `Exec` 指向包装脚本（含预检），而非直接跑 console.py
- `Terminal=false`、`StartupWMClass=console.py`（窗口正确归类）
- 已通过 `desktop-file-validate`（无警告）

> ⚠️ 若程序目录变动（如重命名），需同步更新 `.desktop` 里的 `Exec`/`Path`——v2.0 下重跑 `sudo bash install.sh` 即可整体重新生成（动态 `APP_DIR`）。/ *If the program directory changes (e.g. renamed), update `Exec`/`Path` in `.desktop` — under v2.0, simply re-running `sudo bash install.sh` regenerates them all (dynamic `APP_DIR`).*
...
sudo bash install.sh      # v2.0 动态部署（需 root 装服务/sudoers；无 --check 参数）| dynamic deploy (root needed for services/sudoers; no --check flag)
```
...
> 🟩 2026-09-10 注记 / Note 2026-09-10: 归档真源路径已改为 v2.0 目录（见上第 4、8 节注记）。（第 14 节其余迁移历史不变。）/ *The archive source-of-truth path now points to the v2.0 directory (see the notes in sections 4 and 8 above). The rest of section 14's migration history is unchanged.*
...
> 脱敏范围覆盖 23 个源码/文档/desktop 文件的硬编码路径与标识（v2.0 同步后复扫结果，2026-09-10）；`$(hostname)` 等**动态**取值脚本原样保留（运行时会自动获取本机值）。系统日志、`opencode` 会话库、OS 厂商/OEM 产物等一律不进入本仓库（见 `.gitignore`）。/ *Redaction covers hardcoded paths & identifiers across 23 source/doc/desktop files (re-scanned after the v2.0 sync, 2026-09-10); scripts that read values dynamically (e.g. `$(hostname)`) are kept as-is and pick up local values at runtime. System logs, `opencode` session databases and OS-vendor/OEM artifacts never enter the repo (see `.gitignore`).*
