...
param([Parameter(Mandatory=$true)][ValidateSet('A','B','Analyze')][string]$Phase)
...
elseif($Phase -eq 'B'){
    ...
    if($now -ne 70){ Write-Host "FATAL: apply failed, aborting (no state change risk)"; exit 2 }
    $rows=New-Object System.Collections.ArrayList
    try {
...
elseif($Phase -eq 'Analyze'){
    ...
    $b=Import-Csv (Join-Path $dataD 'B.csv')
