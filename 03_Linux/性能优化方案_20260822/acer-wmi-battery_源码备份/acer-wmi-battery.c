...
get_battery_health_control_status(struct battery_info *bat_status)
{
	...
	if (obj->buffer.length != 8) {
		pr_err("WMI battery status call returned a buffer of "
		       "unexpected length %d\n", obj->buffer.length);
		/* 调试: 打印固件实际返回内容（本机固件返回 4 字节 BEBI） */
		if (obj->buffer.length > 0) {
			u8 *p = obj->buffer.pointer;
			pr_info("BEBI dump (%d bytes): %02x %02x %02x %02x %02x %02x %02x %02x\n",
				obj->buffer.length,
				obj->buffer.length > 0 ? p[0] : 0,
				obj->buffer.length > 1 ? p[1] : 0,
				obj->buffer.length > 2 ? p[2] : 0,
				obj->buffer.length > 3 ? p[3] : 0,
				obj->buffer.length > 4 ? p[4] : 0,
				obj->buffer.length > 5 ? p[5] : 0,
				obj->buffer.length > 6 ? p[6] : 0,
				obj->buffer.length > 7 ? p[7] : 0);
		}
		kfree(obj);
		return AE_ERROR;
	}
...
static int __init acer_battery_init(void)
{
	...
	/* 确认温度通道可用（method 19 = WMBE Case 0x13, SBS 索引 0x08） */
	{
		u32 tmp = 0;
		acpi_status s = get_battery_information(0x8, ACER_BATTERY_INDEX, &tmp);
		if (ACPI_SUCCESS(s) && tmp <= U16_MAX && tmp >= 2731)
			pr_info("BAT temperature: %d.%d C\n",
				(tmp - 2731) / 10, (tmp - 2731) % 10);
		else
			pr_warn("BAT temperature query failed: status=0x%x raw=0x%x\n",
				(unsigned)s, tmp);
	}
	...
	if (ACPI_FAILURE(init_state())) {
		/* 本机固件不支持 health/calibration 查询（method 20 返回 4 字节），
		 * 但 method 19 (WMBE Case 0x13) 温度/电压/电流查询原生可用。
		 * 不退出，仅注册只读 temperature 属性。 */
		pr_warn("health/calibration query not supported by firmware, "
			"continuing with read-only temperature interface\n");
	}
