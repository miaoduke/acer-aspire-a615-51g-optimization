...
class BatteryPage(Gtk.Box):
    def __init__(self):
        ...
        head = Gtk.Label(label="电池保养", xalign=0)
        head.get_style_context().add_class("section-title")
        self.pack_start(head, False, False, 0)

        # ---- 状态卡 ----
        cards = Gtk.Box(spacing=10)
        self.health_card = self._make_card("健康度", "—", "%")
        self.cap_card = self._make_card("当前容量", "—", "")
        self.temp_card = self._make_card("电池温度", "—", "°C")
        self.state_card = self._make_card("充电状态", "—", "")
        ...
        # ---- 温度历史 ----
        tlab = Gtk.Label(label="电池温度历史（近 24 小时，每 10 分钟采样）", xalign=0)
        ...
        chlab = Gtk.Label(label="充电曲线（容量% / 电流 mA）", xalign=0)
        ...
        # ---- 浮充统计 ----
        flab = Gtk.Label(label="满电浮充时长（近 7 天）", xalign=0)
        ...
        # ---- 健康趋势 ----
        hlab = Gtk.Label(label="健康度趋势（放电 40-60% 窗口采样，≥12h 间隔）", xalign=0)
        ...
        ulab = Gtk.Label(label="使用统计（续航监测 + 等效循环）", xalign=0)
        ulab.get_style_context().add_class("section-title")
        self.pack_start(ulab, False, False, 0)
        
        urow = Gtk.Box(spacing=20)
        self.usage_current = self._make_stat("本次离电", "—")
        self.usage_today = self._make_stat("今日离电", "—")
        self.usage_week = self._make_stat("近 7 天", "—")
        self.usage_total = self._make_stat("累计离电", "—")
        self.usage_cycles = self._make_stat("等效循环", "—")
        for w in (self.usage_current, self.usage_today, self.usage_week,
                  self.usage_total, self.usage_cycles):
            box2 = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=1)
            t = Gtk.Label(label=w[0])
            t.get_style_context().add_class("card-title")
            v = Gtk.Label(label=w[1])
            v.get_style_context().add_class("detail-value")
            box2.pack_start(t, False, False, 0)
            box2.pack_start(v, False, False, 0)
            setattr(self, w[2], v) if False else None
            urow.pack_start(box2, True, True, 0)
        # 保存值标签引用
        self._usage_labels = {
            "current": self.usage_current[1], "today": self.usage_today[1],
            "week": self.usage_week[1], "total": self.usage_total[1],
            "cycles": self.usage_cycles[1],
        }
        ...
        # ---- 科学统计分析（2026-08-21）----
        alab = Gtk.Label(label="科学统计分析", xalign=0)
        alab.get_style_context().add_class("section-title")
        self.pack_start(alab, False, False, 0)
        
        self.analytics_summary = Gtk.Label(label="分析加载中...", xalign=0)
        ...
        a_btn = Gtk.Button(label="📋 查看完整分析报告")
        a_btn.set_tooltip_text("7 维度科学分析：循环速率/放电深度/续航效率/充电行为/温度/健康度外推/浮充，含科学依据说明")
        ...
        # ---- 保养规则（科学依据） ----
        rlab = Gtk.Label(label="保养规则（系统自动提醒）", xalign=0)
    ...
    def refresh_state(self, d=None, usage_snap=None):
        ...
        if t:
            ...
            tt = _read(THERMAL)
            self.temp_card._val.set_text("%.0f (整机)" % (int(tt) / 1000) if tt else "—")
        ...
        phase = b.get("phase", "")
        # 阶段标注仅在充电时有意义
        ph_txt = {"cc": " · CC恒流", "cv": " · CV恒压(高损伤)"}.get(phase, "") if st == "Charging" else ""
        self.state_card._val.set_text(f"{st} {cap}{ph_txt}")
        # 健康趋势曲线（点 + 趋势线 + 温度标注）
        pts = []
        for r in hrows[1:]:
            try:
                ...
                continue
        if pts:
            self.health_curve.set_data(pts, "时间", "健康度 %", trend=True)
            last = hrows[-1]
            if len(hrows) >= 2:
                p0, p1 = float(hrows[-2][3]), float(hrows[-1][3])
                delta = p1 - p0
                self.health_note.set_text(
                    "最近采样：%s 健康度 %.2f%%（%s°C）｜较上次 %+.2f%% ｜采样 %d 次（容量测量需周-月尺度，趋势仅供参考）"
                    % (last[0], p1, last[1], delta, len(hrows) - 1))
            else:
                self.health_note.set_text("最近采样：%s 健康度 %.2f%%（%s°C）｜需 ≥2 次采样才显示趋势" % (last[0], float(last[3]), last[1]))
        else:
            self.health_note.set_text("暂无健康采样（自动记录条件：放电中且电量 40-60%，间隔 ≥12 小时）")
        ...
        for r in trows[1:]:
            ...
            if now - ts <= 86400:
                try:
                    ...
                    continue
        if tpts:
            self.temp_curve.set_data(tpts, "时间（近24h）", "温度 °C")
        else:
        ...
        for r in frows[1:]:
            try:
                ...
                continue
        if fpts:
            self.float_curve.set_data(fpts, "日期（近7天）", "浮充分钟")
        else:
    ...
    def _refresh_analytics_summary(self):
        ...
        try:
            ...
            self.analytics_summary.set_text(f"分析暂不可用: {e}")
    ...
    def _open_analytics(self, _btn=None):
        ...
        dlg = Gtk.Dialog(title="电池使用 · 科学统计分析报告",
        ...
        dq_txt = (f"数据质量: 使用 {dq.get('usage_days', 0)} 天 · "
                  f"{dq.get('sessions', 0)} 次离电会话 · "
                  f"{dq.get('health_samples', 0)} 次健康采样 · "
                  f"{dq.get('tsv_points_7d', 0)} 个近 7 天监测点")
        ...
        for item in result["items"]:
            ...
            # 分级着色
            grade_label = Gtk.Label(label=f"评价: {item['grade']}", xalign=0)
            try:
            ...
            # 置信度
            conf_label = Gtk.Label(label=f"置信度: {item['confidence']}", xalign=0)
        ...
        # 底部说明
        foot = Gtk.Label(
            label="分析随数据变动自动更新。数据积累越多（离电会话、健康采样、运行时长），"
                  "结论越可靠。所有建议基于公开锂电研究（BU-808 / IOP 2025 / Arrhenius 方程）。",
            xalign=0, wrap=True)
    ...
    def _update_usage_stats(self):
        ...
        try:
            ...
            lab["week"].set_text("%.1f 小时" % snap["week_dc_hours"])
            lab["total"].set_text("%.1f 小时" % snap["total_dc_hours"])
            lab["cycles"].set_text("%.1f 次" % snap["equiv_cycles"])
            ...
            if sessions:
                lines = ["最近离电记录："]
                for s in sessions[:5]:
                    lines.append("  %s · %d 分钟（%d%%→%d%%）" % (
                        s.get("date", "?"), s.get("duration_min", 0),
                        s.get("start_cap", 0), s.get("end_cap", 0)))
