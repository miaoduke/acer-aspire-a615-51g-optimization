...
class BatteryAnalytics:
    ...
    def _grade(self, value_good: bool) -> Tuple[str, str]:
        return ("✓ 良好", "#2e7d32") if value_good else ("⚠ 需注意", "#e65100")
    ...
    def _analyze_efc_rate(self, usage: Dict) -> Dict:
        ...
        if efc_per_day <= 0.05:
            value = f"{efc_per_day:.3f} 次/天（累计 {equiv:.1f} 次）"
            good, note = True, "数据积累中"
        else:
            value = f"{efc_per_day:.2f} 次/天（累计 {equiv:.1f} 次）"
        ...
        if efc_per_day > 0.02:
            ...
            years_to_70pct = f"按此速率约 {days_left/365:.1f} 年后健康度降至 ~70%"

        science = ("等效满循环(EFC)=累计充入电荷÷设计容量。锂电循环寿命与 DoD 强相关:"
        ...
        return {
            ...
            "grade": "✓ 良好" if good else "⚠ 偏高",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("循环速率低 → 电池老化慢。当前速率处于正常范围。"
                            if good else
                            "循环速率偏高（>0.8 EFC/天），建议减少深放电、多用插电。"),
            "science": science,
            "confidence": "高" if days >= 7 and equiv > 1 else "低（数据积累中，≥7 天且 ≥1 循环后有参考价值）",
        }

    def _analyze_dod(self, sessions: List[Dict]) -> Dict:
        """2. 放电深度 (DoD) 分析"""
        if not sessions:
            return self._insufficient("放电深度 (DoD)", "完成 1 次以上离电会话后可用",
                                      "DoD=单次放电的电量百分比。浅放电显著延长循环寿命:")
        ...
        good = avg_dod < 60
        value = f"平均 {avg_dod:.0f}%（{len(dods)} 次会话，深放 {deep_count} 次）"

        science = ("DoD 与循环寿命近似幂律关系(BU-808 实测): 100% DoD≈300-500 次、"
        ...
        return {
            "title": "放电深度 (DoD)",
            "value": value,
            "grade": "✓ 良好" if good else "⚠ 偏深",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("平均放电深度适中，浅充放习惯有利寿命。"
                            if good else
                            "平均放电较深。建议电量低于 25-30% 即充电，避免用到 15% 以下。"),
            "science": science,
            "confidence": "中" if len(sessions) >= 3 else "低（≥3 次会话后更准）",
        }
    ...
    def _analyze_endurance_efficiency(self, sessions: List[Dict], usage: Dict) -> Dict:
        ...
        if len(valid) < 2:
            return self._insufficient(
                "续航效率趋势", "完成 2 次以上有效离电会话（≥20 分钟、耗电 ≥10%）后可用",
                "健康衰减的直接体感: 同样 100%→30%,使用时间越来越短。"
        ...
        value = f"最新 {latest:.0f} 分钟/100% vs 基线 {baseline:.0f} 分钟/100%"
        if full_wh:
            value += f"\n当前满充 ≈ {full_wh:.1f} Wh"
        ...
        if abs(change_pct) < 8:
            ...
            trend_txt = f"下降 {-change_pct:.0f}%"
            good = False
        else:
            trend_txt = f"提升 {change_pct:+.0f}%（负载差异所致）"
            good = True

        science = ("续航效率 = 会话时长 ÷ 耗电百分比,即折算 100% 电量的可用时长。"
        ...
        return {
            "title": "续航效率趋势",
            "value": value + f"\n趋势: {trend_txt}",
            "grade": "✓ 稳定" if good else "⚠ 下降",
            "grade_color": "#2e7d32" if good else "#c62828",
            "explanation": ("效率稳定,无明显衰减迹象。" if good else
                            "效率呈下降趋势——可能为健康衰减或近期负载更高,继续观察。"),
            "science": science,
            "confidence": "中" if len(valid) >= 4 else "低（会话越多越准）",
        }
    ...
    def _analyze_charge_behavior(self, batt_rows: List[Dict]) -> Dict:
        ...
        if len(charging) < 10:
            return self._insufficient(
                "充电行为", "插电使用积累更多采样后可用",
                "充电行为影响寿命的两个维度: 充电时电池温度、以及是否长期维持高电量(浮充)。")
        ...
        value = (f"充电均温 {avg_temp:.1f}°C | ≥40°C 占比 {high_temp_ratio:.0f}%\n"
                 f"高电量(≥95%)时段占比 {high_cap_ratio:.0f}%")

        science = ("充电过程本身产热(内阻损耗)+高 SoC 电压应力是两大老化因素:"
        ...
        return {
            "title": "充电行为（近 7 天）",
            "value": value,
            "grade": "✓ 良好" if good else "⚠ 注意",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("充电温度与电量区间健康。" if good else
                            "存在高温充电或长时间满电情况——若常插电使用，"
    ...
    def _analyze_temperature(self, batt_rows: List[Dict]) -> Dict:
        ...
        if len(batt_rows) < 20:
            return self._insufficient(
                "温度暴露", "控制台运行积累更多采样后可用",
                "Arrhenius 定律: 温度每升 10°C,电化学副反应速率约提升一倍,"
        ...
        value = f"均值 {avg:.1f}°C | 峰值 {mx:.0f}°C | ≥40°C 占比 {hot_ratio:.0f}%"

        science = ("锂电池理想工作温度 15-35°C(BU-808)。温度对老化的影响符合 Arrhenius "
        ...
        return {
            "title": "温度暴露（近 7 天）",
            "value": value,
            "grade": "✓ 良好" if good else "⚠ 偏热",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("电池温度控制在理想区间。" if good else
                            "电池环境偏热——检查通风、避免阳光直晒、"
                            "高负载时注意散热口不被遮挡。"),
            "science": science,
            "confidence": "中",
    ...
    def _analyze_health_trend(self, health_rows: List[Dict]) -> Dict:
        ...
        if len(health_rows) < 2:
            return self._insufficient(
                "健康度趋势外推", "需 ≥2 次协议采样（放电 40-60% 自动记录，间隔 ≥12h）",
                "健康度 = 当前满充容量 ÷ 设计容量。周期性采样做线性回归,"
        ...
        value = f"当前 {current:.1f}% | 斜率 {slope:+.3f}%/天（{n} 次采样，跨 {span_days} 天）"

        extrapolate = ""
        if slope < -0.005:  # 明显下降
            days_to_80 = (current - 80) / (-slope)
            days_to_70 = (current - 70) / (-slope)
            extrapolate = (f"\n外推: 约 {days_to_80:.0f} 天后到 80%，"
                           f"{days_to_70:.0f} 天后到 70%" if current > 80 else "")
        elif slope >= 0:
            extrapolate = "\n未见下降趋势（或测量噪声掩盖）"

        good = slope > -0.02  # 每天掉 <0.02% 属于缓慢
        science = ("健康度采样在放电 40-60% 窗口进行以减少读数噪声。线性回归外推的前提是"
        ...
        return {
            ...
            "grade": "✓ 缓慢" if good else "⚠ 偏快",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("衰减速度缓慢,按当前趋势无需担心。" if good else
                            "衰减偏快——结合 DoD/温度/循环速率几项找原因。"),
            "science": science,
            "confidence": "高" if n >= 5 and span_days >= 14 else ("中" if n >= 3 else "低"),
        }
    ...
    def _analyze_float(self, float_min: Optional[int]) -> Dict:
        ...
        if float_min is None:
            return self._insufficient(
                "浮充管理", "battery-care 服务运行后可用",
                "浮充=满电状态下继续插电。高 SoC+持续外接电源使电池长期处于 4.2V/C 高电压应力,"
                "是最伤电池的使用模式之一。")
        
        hours = float_min / 60
        good = hours < 120  # 今日浮充 <2h
        value = f"今日浮充 {hours:.1f} 小时"
        science = ("满电(4.2V/cell)持续浮充时,负极 SEI 持续增厚、电解液氧化分解加速。"
        ...
        return {
            "title": "浮充管理（今日）",
            "value": value,
            "grade": "✓ 正常" if good else "⚠ 浮充多",
            "grade_color": "#2e7d32" if good else "#e65100",
            "explanation": ("浮充时长可控。" if good else
                            "今日浮充较长。插电为主的使用建议设置充电上限。"),
            "science": science,
            "confidence": "高",
        }
    ...
    def _build_summary(self, items: List[Dict], dq: Dict) -> str:
        ...
        if dq.get("sufficient"):
            head = f"综合评价: {ok} 项良好 / {warn} 项需注意 / {pending} 项待数据"
        else:
            head = (f"数据积累期（使用 {dq.get('usage_days', 0)} 天、"
                    f"{dq.get('sessions', 0)} 次会话）——多数分析将在数据充足后自动给出结论")
        return head
...
if __name__ == '__main__':
    ...
    for item in result["items"]:
        ...
        print(f"  解读: {item['explanation']}")
        print(f"  科学依据: {item['science'][:80]}...")
