...
- **当前定稿: -80mV**（core/cache/gpu 三域，实测 -80.08mV）✅ **已持久化**
- **历史**: -50mV（多日稳定）→ **-80mV**（2026-08-30 五项场景验证全过，MCE 全 0）
- **-105mV**: 死机 #6（`kernel panic` MCE 广播超时）→ **禁止**；建议止步 -100mV
- **观察期**: -80mV 设 3-5 天（至 09-03），由 `uv-daily-check.timer` 自动巡检
- **安全网**: `uv-safeguard` 异常关机自动回退 -50mV
