#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ui_docs 双语接入（重建版）：DOC_EN 380 条对照 + _D() + 7 入口方法包裹"""
import ast
import json

# ============ DOC_EN 对照表（zh 原文 → en 译文，380 条）============
DOC_EN = {
...
# ============ 写入 ui_docs.py ============
src = open('ui_docs.py', encoding='utf-8').read()

cls_anchor = "class DocumentationPage(Gtk.Box):"
assert cls_anchor in src

d_func = "# ---------------- 2026-09-08: 文档正文双语（en locale 显示英文版）----------------\n"
d_func += "# DOC_EN: 文档正文 zh→en 对照表（独立于主 i18n 字典——长文档文案不入 T() 字典\n"
d_func += "# 是科学决策：避免主字典膨胀；zh 原文为键，en locale 时 _D() 查表，未收录回退原文）\n"
d_func += "DOC_EN = {\n"
for k, v in DOC_EN.items():
    d_func += f"    {k!r}: {v!r},\n"
d_func += "}\n\n\n"
d_func += "def _D(text):\n"
d_func += '    """文档正文双语：en locale 时查 DOC_EN，否则原文"""\n'
d_func += "    from src.core.i18n import I18n\n"
d_func += "    loc = I18n.get()._current_locale or 'zh_CN'\n"
d_func += "    if loc == 'en_US':\n"
d_func += "        return DOC_EN.get(text, text)\n"
d_func += "    return text\n\n\n"

src = src.replace(cls_anchor, d_func + cls_anchor, 1)
...
for old, new in wraps:
    if old in src:
        ...
        print(f"NOT FOUND: {old.splitlines()[0]}")
print(f"wrapped {n}/7 entry methods")

ast.parse(src)
open('ui_docs.py', 'w', encoding='utf-8').write(src)
print(f"DOC_EN {len(DOC_EN)} entries written into ui_docs.py")

# 覆盖率核查
import json
zh = json.load(open('/tmp/docs_zh_strings.json', encoding='utf-8'))
missing = [s for s in zh if s not in DOC_EN]
print(f"coverage: {len(zh)-len(missing)}/{len(zh)}")
for m in missing[:8]:
