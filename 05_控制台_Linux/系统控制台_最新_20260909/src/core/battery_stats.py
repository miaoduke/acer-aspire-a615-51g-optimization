...
DATA_FILE = Path(os.path.expanduser("~/.local/share/系统控制台/data/battery_usage.json"))
...
class BatteryUsageTracker:
    ...
    def _save(self):
        ...
        try:
            ...
            # 2026-09-07 审计：持久化失败不能静默（电池统计丢失无感知，P0-A 同源风险）
            # 限频告警：避免每 tick 刷屏，但首次失败必报
            import time as _t
            now = _t.time()
            cls = type(self)
            if now - getattr(cls, '_last_save_err', 0) > 600:
                cls._last_save_err = now
                print(f"[battery-stats] 持久化失败(10 分钟内静默): {e}")
                ...
                if last is not None and charge_now > last:
                    ...
                    if 0 < delta_uah < 500000:
                        ...
                        if self._data.get("design_uwh"):
                            # 2026-09-01 修复: 直接用硬件设计容量(charge_full_design, µAh)，
                            # 避免电压换算偏差(本机 4S 电池满充~16.8V, 旧代码默认 11.6V=3S 假设，
                            # 电压读失败时设计容量被高估 ~46% → 等效循环被低估)
                            design_uah_approx = design_uah or (self._data["design_uwh"] * 1e6 // max(1, volt_uv_safe()))
    ...
    def _end_session(self, end_time: float, end_cap: int):
        ...
        session = {
            ...
            # 2026-09-01 修复: end_cap 原取插电瞬间容量(充电器接入后容量计可能跳升, 如 97→100)。
            # 离电时电量只降不升 → 会话最低电量即真实结束电量
            "end_cap": self._session_min_cap,
    ...
    def format_summary(self, snap: Dict) -> str:
        ...
        if not snap["is_ac"]:
            ...
            lines.append(T("本次离电：{}").format(fmt_dur(m * 60)))
        lines.append(T("今日离电：{}").format(fmt_dur(snap["today_dc_min"] * 60)))
        lines.append(T("近 7 天：{}").format(fmt_dur(snap["week_dc_hours"] * 3600)))
        lines.append(T("累计离电：{}").format(fmt_dur(snap["total_dc_hours"] * 3600)))
        lines.append(T("等效循环：%.1f 次（自统计 %.1f Ah）") % (
