...
## 降压验收记录（2026-09-13，重装恢复后重新验收）

**-100mV（三域 core/cache/gpu）正式验收通过**，观察期语义解除：
- 观察期：2026-09-11 14:19 应用，至 2026-09-13 11:27 共 45h，期间 4 次成功应用记录（uv_set.log）
- 满载验收：360,446,487 迭代 / 1802.2 万/s（超历史全部记录，含 -50 时代 1779 万/s、m105 扫描 1407-1532 万/s）
- 硬件证据：MCE/Memory/PCIe AER/Extlog 四类零错误（rasdaemon 在岗），uv_safeguard verdict=CLEAN
- 满载包温 71°C（历史同负载 81-86°C 区间下限）
- **安全网同步升级：SAFE_MV 50 → 80**（uv_safeguard.sh，异常关机回退目标改为稳定档下方一档，已部署 /usr/local/bin 双边一致）
- 下探路线：下一档 -105（历史 m105 有 9 轮数据支持），24h 观察期
