...
EOF
  ...
  # 2026-09-01 补: acdc-profile 冷启动曾设 hwp_dynamic_boost=1(插电性能项), M3 下应归 0(IO 提频费电)
  echo 0 > /sys/devices/system/cpu/intel_pstate/hwp_dynamic_boost 2>/dev/null
  echo "✅ M3 已生效（EPP=power，acdc-profile 已停，PL1=10W，Turbo 关，hwp=0，重启后保持）"
...
mode_off() {
  ...
  # 真实用户家目录（可能被 sudo 运行，$HOME=/root 需纠正为实际用户）
