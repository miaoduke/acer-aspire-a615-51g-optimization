...
kv "MCE 计数" "$(journalctl -b -k 2>/dev/null | grep -iE 'machine check|hardware error|mce.*bank' | grep -ivE 'mcelog|rasdaemon|event enabled|hash matches|aer|pcie|dellp|undervolt|write to unrecognized' | wc -l) (真实 MCE；2026-09-04 P0-1 修复：旧 grep 'mce' 会把 mcelog 注册行误计入)"
