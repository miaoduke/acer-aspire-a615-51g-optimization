...
class AdvancedPage(Gtk.Box):
    def __init__(self):
        ...
        head = Gtk.Label(label=T("高级控制"), xalign=0)
        ...
        note = Gtk.Label(
            label=T("提示：PL/Turbo/EPP/Governor 的调整请在【电源场景】页一键切换（已验证脚本）。"),
            xalign=0, wrap=True)
        ...
        glab = Gtk.Label(label=T("GPU 模式："), xalign=0)
        self.gpu_intel = Gtk.Button(label=T("切到集显 intel（省电 ~7.7W）"))
        self.gpu_intel.connect("clicked", self._on_gpu, "intel")
        self.gpu_nvidia = Gtk.Button(label=T("切到独显 nvidia（性能）"))
        ...
        self.tcc_apply = Gtk.Button(label=T("应用并验证"))
        ...
        pll = Gtk.Label(label=T("PL1/PL2 功耗墙："), xalign=0)
        ...
        self.pl_apply = Gtk.Button(label=T("应用并验证"))
        ...
        fql = Gtk.Label(label=T("CPU 频率上限："), xalign=0)
        ...
        self.fq_apply = Gtk.Button(label=T("应用并验证"))
        ...
        pcl = Gtk.Label(label=T("CPU 强制降载："), xalign=0)
        ...
        self.pc_apply = Gtk.Button(label=T("应用并验证"))
        ...
        tnote = Gtk.Label(
            label=T("提示：温度墙越大 CPU 越早降频（静音模式适用）；降压/功耗控制均写 sysfs，不经 EC。"),
            xalign=0, wrap=True)
        ...
        plab = Gtk.Label(label=T("外设省电："), xalign=0)
        self.usb_save = Gtk.Button(label=T("USB 省电 开"))
        self.usb_save.connect("clicked", self._on_usb, True)
        self.usb_full = Gtk.Button(label=T("USB 常供电"))
        self.usb_full.connect("clicked", self._on_usb, False)
        self.wifi_save = Gtk.Button(label=T("WiFi 省电 开"))
        self.wifi_save.connect("clicked", self._on_wifi, True)
        self.wifi_perf = Gtk.Button(label=T("WiFi 性能"))
        self.wifi_perf.connect("clicked", self._on_wifi, False)
        self.cam_on = Gtk.Button(label=T("摄像头 开"))
        self.cam_on.connect("clicked", self._on_camera, True)
        self.cam_off = Gtk.Button(label=T("摄像头 关"))
        ...
        el = Gtk.Label(label=T("生态优化状态"), xalign=0)
        ...
        erows = [
            ("scene", T("场景自动切换"), T("acdc-profile 服务")), ("bright", T("屏幕亮度"), T("用户自主控制")),
            ("cstate", T("C-state 上限"), "intel_idle.max_cstate"), ("mx150", T("MX150 断电"), T("D3cold 状态")),
            ("zswap", "zswap", T("内存压缩交换")), ("shrinker", "zswap shrinker", T("冷页提前驱逐")),
            ("swap", T("swap 文件"), T("4G btrfs 兜底")), ("hwp_boost", "hwp_dynamic_boost", T("IO 唤醒提频")),
            ("discharge", T("电池放电率"), "current×voltage"), ("smartd", "smartd", T("双盘 SMART 自检")),
            ("jlimit", T("journald 限容"), T("日志 100M 上限")), ("startup", T("启动优化"), "wait-online/fwupd"),
            ("undervolt", "undervolt", T("CPU/GPU 降压")), ("gtfreq", T("GPU GT 频率"), T("AC/DC 联动")),
        ]
        ...
        tl2 = Gtk.Label(label=T("工具与配置"), xalign=0)
        tl2.get_style_context().add_class("section-title")
        tbox2.pack_start(tl2, False, False, 0)
        
        btn_row = Gtk.Box(spacing=8)
        
        guard_btn = Gtk.Button(label=T("🌡️ 温度守护配置"))
        guard_btn.set_tooltip_text(T("配置 thermal-guard 高温触发/恢复阈值和限流 PL1 值"))
        guard_btn.connect("clicked", self._open_guard_config)
        
        scene_btn = Gtk.Button(label=T("⚙️ 场景参数"))
        scene_btn.set_tooltip_text(T("配置插电/离电默认场景和 GPU GT 频率联动值"))
        scene_btn.connect("clicked", self._open_scene_config)

        adapt_btn = Gtk.Button(label=T("🖥 跨机适配"))
        adapt_btn.set_tooltip_text(T("检测硬件能力 + 生成兼容报告（重装/换同配置机器后用）"))
        adapt_btn.connect("clicked", self._on_adapt)

        win_btn = Gtk.Button(label=T("🪟 临时切到 Windows"))
        win_btn.set_tooltip_text(T("UEFI BootNext 临时启动 Windows（重启生效，下次自动回 Linux）"))
        win_btn.connect("clicked", self._on_boot_windows)

        tools_btn = Gtk.Button(label=T("🧰 高级工具"))
        tools_btn.set_tooltip_text(T("GRUB 修改 / 降压应用 / 降压扫描等高风险操作（命令行）"))
    ...
    def _on_gpu(self, _btn, target):
        ...
        def done(result):
            ...
            if rc == 0:
                d.set_markup(T('<b>GPU 已切换为 %s</b>\n\n配置已写入，需<b>重启系统</b>后完全生效。\n恢复独显：控制台 GPU 模式 → 切到 nvidia。') % target)
                d.add_button(T("稍后重启"), Gtk.ResponseType.NO)
                b_reboot = d.add_button(T("立即重启"), Gtk.ResponseType.YES)
                b_reboot.get_style_context().add_class("suggested-action")
            else:
                d.set_markup(T("<b>切换失败</b>\n%s") % (err or out))
                d.add_button(T("确定"), Gtk.ResponseType.OK)
            resp = d.run()
            d.destroy()
    ...
    def _on_apply(self, _btn, kind):
        def worker():
            try:
                ...
                return (1, "", T("请输入整数（%s-%s）") % rng)
            if not (rng[0] <= v <= rng[1]):
                return (1, "", T("数值超出范围（%s-%s）") % rng)
            # 同步滑块/填写框
        ...
        def done(result):
            ...
            if rc in (0, 2):
                d.set_markup("<b>%s</b>\n\n%s" % (
                    T("设置已应用并验证生效") if rc == 0 else T("已写入但实测不一致"),
                    out.strip() or T("（无输出）")))
            else:
                d.set_markup(T("<b>操作失败</b>（rc=%s）\n%s") % (rc, err or out))
            d.run()
    ...
    def _on_pl_apply(self, _btn):
        ...
        def done(result):
            ...
            if rc in (0, 2):
                d.set_markup("<b>%s</b>\n\n%s" % (
                    T("功耗墙已应用并验证生效") if rc == 0 else T("已写入但实测不一致"),
                    out.strip() or T("（无输出）")))
            else:
                d.set_markup(T("<b>操作失败</b>（rc=%s）\n%s") % (rc, err or out))
            d.run()
    ...
    def _on_camera(self, _btn, on):
        ...
        def done(result):
            ...
            if rc == 0:
                ...
                state_txt = T("已启用（uvcvideo 已加载）") if status else T("已禁用（uvcvideo 已卸载）")
                d.set_markup(
                    T('<b>摄像头已%s</b>\n\n验证：%s %s\n%s') % (
                        T("启用") if on else T("禁用"),
                        T("✓ 驱动状态符合") if ok else T("⚠ 驱动状态与目标不符"),
                        state_txt,
                        T("提示：正在使用摄像头的程序需重启才能重新识别") if on else
                        T("提示：隐私保护——系统级禁用，任何程序都无法调用摄像头")))
            else:
                d.set_markup(T("<b>操作失败</b>（rc=%s）\n%s") % (rc, err or out))
            d.run()
            d.destroy()
    ...
    def refresh_cam_state(self):
        ...
        def done(s):
            self.cam_state.set_text(T("当前：%s") % (T("启用") if s else T("禁用")))
        run_async(worker, done)
    ...
    def refresh_state(self, d=None):
        ...
        if p["pl1_w"] is not None and p["pl2_w"] is not None:
            ...
            self.pl_state.set_text(T("当前生效：PL1=%dW PL2=%dW") % (round(p["pl1_w"]), round(p["pl2_w"])))
        ...
        if tcc is not None:
            ...
            self.tcc_state.set_text(T("当前生效：%d°C") % tcc)
        ...
        if eco:
            ...
            scene_txt = {"active": T("AC/DC 自动切换"), "inactive": T("已停（M3 覆盖）"),
                         "failed": T("失败"), "unknown": T("未知")}.get(scene, scene or T("未知"))
            self.eco["scene"][0].set_text(scene_txt)
            # 亮度：只读显示（用户自主控制，acdc-profile 不再联动）
            bmax = eco.get("brightness_max")
            bcur = eco.get("brightness")
            if bcur and bmax:
                self.eco["bright"][0].set_text(T("%d%%（%s/%s）· 用户自主") % (
                    int(bcur) * 100 // int(bmax), bcur, bmax))
            else:
                self.eco["bright"][0].set_text("—")
            self.eco["cstate"][0].set_text(eco.get("max_cstate") or T("默认"))
            mx = eco.get("mx150")
            self.eco["mx150"][0].set_text(
                T("断电（D3cold）✓") if mx == "suspended" else (mx or "—"))
            zs = eco.get("zswap")
            self.eco["zswap"][0].set_text(
                T("开启 ✓") if zs and zs.lower() in ("y", "1") else T("关闭"))
            sh = eco.get("zswap_shrinker")
            self.eco["shrinker"][0].set_text(
                T("开启 ✓") if sh and sh.lower() in ("y", "1") else T("关闭"))
            self.eco["swap"][0].set_text(eco.get("swap") or T("无"))
            hb = eco.get("hwp_boost")
            self.eco["hwp_boost"][0].set_text(
                T("开启 ✓") if hb == "1" else (T("关闭") if hb == "0" else "—"))
            ...
            self.eco["smartd"][0].set_text(
                T("监控中 ✓") if smart == "active" else (smart or "—"))
            jd = eco.get("journald_max") or ""
            jm = "100M" if "SystemMaxUse=100M" in jd else T("默认")
            ...
            if uv == "active":
                ...
                off = ("%smV" % m.group(1)) if m else (T("已配置") if cfg else "")
                self.eco["undervolt"][0].set_text(T("生效 ✓ %s") % off)
            else:
                self.eco["undervolt"][0].set_text(uv or "—")
            gtmax = eco.get("gt_max")
            gtrp0 = eco.get("gt_rp0")
            if gtmax and gtrp0:
                mark = T("（AC 全速）") if d.get("ac") else T("（DC 受限 700）")
    ...
    def _m3_state():
        try:
            ...
            return T("生效中") if r.stdout.strip() == "active" else T("未启用")
        except Exception:
            return "—"
    ...
    def _open_guard_config(self, _btn=None):
        ...
        dlg = Gtk.Dialog(title=T("温度守护阈值配置"), transient_for=self.get_toplevel(), modal=True)
        ...
        try:
            with open(os.path.join(BASE, "backend", "thermal_guard.sh")) as f:
                content = f.read()
        ...
        lab1 = Gtk.Label(label=T("高温触发 (°C)："), xalign=0)
        ...
        lab2 = Gtk.Label(label=T("恢复阈值 (°C)："), xalign=0)
        ...
        lab3 = Gtk.Label(label=T("限流 PL1 (W)："), xalign=0)
        ...
        note = Gtk.Label(label=T("说明：温度超过高温触发值时自动降 PL1 至限流值；\n回落到恢复值以下时自动恢复原 PL1。"), xalign=0)
        note.get_style_context().add_class("dim-text")
        box.pack_start(note, False, False, 0)

        # 按钮
        btn_box = Gtk.Box(spacing=8)
        apply_btn = Gtk.Button(label=T("应用"))
        apply_btn.get_style_context().add_class("suggested-action")
        cancel_btn = Gtk.Button(label=T("取消"))
        ...
        def on_apply(_btn):
            ...
            def worker():
                import subprocess
                script = os.path.join(BASE, "backend", "thermal_guard.sh")
                # 用 sed 更新脚本中的默认值
...
systemctl restart thermal-guard"""
            ...
            def done(result):
                ...
                d = Gtk.MessageDialog(transient_for=dlg, modal=True,
                                      ...
                                      buttons=Gtk.ButtonsType.OK)
                if rc == 0:
                    d.set_markup(T("<b>✓ 阈值已更新并重启服务</b>"))
                    dlg.destroy()
                else:
                    d.set_markup(T("<b>更新失败</b>\n{}").format(err or out))
                d.run()
                d.destroy()
    ...
    def _on_adapt(self, _btn=None):
        ...
        dlg.add_button(T("取消"), Gtk.ResponseType.NO)
        b_check = dlg.add_button(T("仅检测"), Gtk.ResponseType.CLOSE)
        b_check.get_style_context().add_class("suggested-action")
        b_apply = dlg.add_button(T("检测+应用"), Gtk.ResponseType.YES)
        resp = dlg.run()
        dlg.destroy()
        ...
        def done(result):
            ...
            d = Gtk.MessageDialog(
                ...
                message_type=Gtk.MessageType.INFO if rc == 0 else Gtk.MessageType.ERROR,
                buttons=Gtk.ButtonsType.OK)
            d.set_markup(T("<b>跨机适配报告</b>\n\n%s") % (T("（输出见下方）") if rc == 0 else T("（失败）")))
            sw = Gtk.ScrolledWindow()
            sw.set_size_request(600, 360)
            buf = Gtk.TextBuffer()
            buf.set_text((out or T("(无输出)")) + ((T("\n\n[错误] ") + err) if err and rc != 0 else ""))
    ...
    def _on_boot_windows(self, _btn=None):
        ...
        dlg.add_button(T("取消"), Gtk.ResponseType.NO)
        b_go = dlg.add_button(T("设置并重启进 Windows"), Gtk.ResponseType.YES)
        ...
        def done(result):
            ...
            d = Gtk.MessageDialog(
                ...
                buttons=Gtk.ButtonsType.OK)
            if rc == 0:
                d.set_markup(T("<b>已排程启动 Windows</b>\n\n%s") % (out or T("重启后将进入 Windows")))
            else:
                d.set_markup(T("<b>设置失败</b>\n%s") % (err or out))
            d.run()
            d.destroy()
    ...
    def _on_advanced_tools(self, _btn=None):
        ...
        TOOLS = [
            (T("GRUB 参数修改（C-state/性能参数，需重启生效）"),
             "sudo nano /etc/default/grub && sudo update-grub"),
            (T("降压应用（当前 -80mV 由服务管理，改动需谨慎）"),
             "sudo systemctl edit undervolt.service\nsudo systemctl restart undervolt"),
            (T("降压扫描（高风险实验，必须带防护）"),
             T("sudo bash 性能优化方案_20260822/测量脚本/uv_sweep_v2.sh 100 5")),
            (T("电池温度模块注册（DKMS，内核升级后自动重建）"),
             T("sudo bash 性能优化方案_20260822/acer-wmi-battery_源码备份/register_dkms.sh")),
            (T("真值采集（核对系统实际状态）"),
             T("sudo bash 05_控制台_Linux/系统控制台_最新_20260822/backend/collect_ground_truth.sh")),
        ]
        dlg = Gtk.Dialog(title=T("高级工具（高风险操作）"), transient_for=self.get_toplevel(),
        ...
        sw.add(tv)
        box.pack_start(sw, True, True, 0)
        dlg.add_button(T("知道了"), Gtk.ResponseType.OK)
        dlg.show_all()
        dlg.run()
        dlg.destroy()
    ...
    def _open_scene_config(self, _btn=None):
        """打开场景参数配置对话框"""
        dlg = Gtk.Dialog(title=T("场景参数配置"), transient_for=self.get_toplevel(), modal=True)
        ...
        scene_path = os.path.expanduser("~/.config/system-console/default_scene")
        config = {}
        try:
        ...
        # 插电默认场景
        lab_ac = Gtk.Label(label=T("插电默认场景："), xalign=0)
        ...
        # 离电默认场景
        lab_dc = Gtk.Label(label=T("离电默认场景："), xalign=0)
        ...
        # GPU AC 频率
        lab_ga = Gtk.Label(label=T("GPU AC 频率 (MHz)："), xalign=0)
        ...
        # GPU DC 频率
        lab_gd = Gtk.Label(label=T("GPU DC 频率 (MHz)："), xalign=0)
        ...
        note = Gtk.Label(label=T("说明：修改后写入 default_scene，下次插拔电时生效。\nGT 频率立即生效。"), xalign=0)
        note.get_style_context().add_class("dim-text")
        box.pack_start(note, False, False, 0)

        # 按钮
        btn_box = Gtk.Box(spacing=8)
        apply_btn = Gtk.Button(label=T("保存"))
        apply_btn.get_style_context().add_class("suggested-action")
        cancel_btn = Gtk.Button(label=T("取消"))
        btn_box.pack_end(apply_btn, False, False, 0)
        btn_box.pack_end(cancel_btn, False, False, 0)
...
"""
            ...
            def done(result):
                ...
                d = Gtk.MessageDialog(transient_for=dlg, modal=True,
                                      ...
                                      buttons=Gtk.ButtonsType.OK)
                if rc == 0:
                    d.set_markup(T("<b>✓ 场景参数已保存</b>\n\n下次插拔电时自动应用新配置"))
                    d.run(); d.destroy()
                    dlg.destroy()
                else:
                    d.set_markup(T("<b>保存失败</b>\n{}").format(err or out))
                    d.run(); d.destroy()
